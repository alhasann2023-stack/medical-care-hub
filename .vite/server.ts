...
async function loadCredentialsAndUsersFromFirestore() {
  try {
    const [fsUsers, fsCreds] = await Promise.all([
...
export function createApiApp() {
  ...
  app.post('/api/appointments', (req: Request, res: Response) => {
    ...
    const initialPaymentStatus = isWaived ? 'WAIVED' : (isPaid ? 'PAYMENT_SUCCESS' : 'PAYMENT_REQUIRED');
    const initialStatus = isWaived ? 'PENDING' : (isPaid ? 'CONFIRMED' : 'PAYMENT_REQUIRED');
    ...
    const newAppointment: Appointment = {
      ...
      coordinatorNotes: isWaived ? 'طلب جديد بإعفاء مالي - بانتظار اتصال منسق خدمة العملاء.' : (isPaid ? 'تم سداد رسوم الحجز إلكترونياً وتأكيد الموعد.' : 'طلب جديد بانتظار سداد الرسوم من المريض.'),
  ...
  app.post('/api/consultations', (req: Request, res: Response) => {
    ...
    const isFreePromoActive = isFreeConsultationActive(docId);
    ...
    if (isFreePromoActive) {
      finalIsWaived = true;
      fee = 0;
      waiverReason = `مبادرة استشارة مجانية بمناسبة: ${freeConsultationPromo.occasionTitle}`;
    ...
    const isPaid = Boolean(req.body.isPaid || req.body.paymentId);
    const initialPaymentStatus = finalIsWaived ? 'WAIVED' : (isPaid ? 'PAYMENT_SUCCESS' : 'PAYMENT_REQUIRED');
    const initialStatus = finalIsWaived ? 'PENDING' : (isPaid ? 'PAID_PENDING_DOCTOR' : 'PAYMENT_REQUIRED');
  ...
  app.post('/api/tests', (req: Request, res: Response) => {
    ...
    const actorRole = (req.body.labTechnician ? 'LAB_TECHNICIAN' : 'DOCTOR') as UserRole;
    const actorName = req.body.labTechnician || doctor.fullName;
    logAudit(actorName, actorName, actorRole, 'ADD_TEST_RESULT', 'TEST', newTest.id, `إرسال نتائج فحص [${testName}] للمريض ${patient.fullName} إلى الطبيب المعالج ${doctor.fullName}`, req);

    // Notify Patient
    pushNotification(
      patient.userId,
      'نتيجة فحص طبي جديدة',
      `تم إصدار نتيجة فحص "${testName}" في ملفك الطبي وإرسالها لطبيبك المعالج ${doctor.fullName}.`,
      'TEST_RESULT',
      newTest.id
    );
    ...
    if (doctor?.userId) {
      pushNotification(
        doctor.userId,
        'نتائج فحص مخبري جديدة لمريضك',
        `قام قسم المختبر (${newTest.labTechnician || 'المختبر'}) بإرسال نتائج فحص "${testName}" للمريض ${patient.fullName}.`,
        'TEST_RESULT',
        newTest.id
      );
    }
  ...
  app.post('/api/appointments/notify-doctor-absent', (req: Request, res: Response) => {
    ...
    targetAppointments.forEach(apt => {
      ...
      const notifMessage = customMessage?.trim() || 
        `نود إحاطتكم بأن الطبيب ${docName} غير مداوم في العيادة بتاريخ ${aptDate} لظرف طارئ. نرجو عدم الحضور إلى العيادة حرصاً على راحتكم ووقتكم، وسيقوم فريق خدمة العملاء بالتواصل معكم هاتفياً لترتيب موعد بديل يناسبكم.`;
  ...
  app.post('/api/admin/staff', async (req: Request, res: Response) => {
    ...
    const assignedRole = (isLab ? 'LAB_TECHNICIAN' : (isSecretary ? 'SECRETARY' : 'CUSTOMER_SERVICE')) as UserRole;

    const assignedAvatar = isLab
    ...
    const newStaff: Staff = {
      ...
      phone: normalizedPhone,
      email: normalizedEmail,
      department: department || (isLab ? 'قسم المختبر والتحاليل الطبية' : (isSecretary ? 'مكتب السكرتاريا والاستقبال' : 'مركز خدمة وتنسيق المواعيد')),
      roleTitle: roleTitle || (isLab ? 'أخصائي / فني مختبر وتحاليل' : (isSecretary ? 'سكرتير طبي واستقبال' : 'منسق خدمة عملاء ورعاية المرضى')),
    ...
    const roleNameAr = isLab ? 'فني وأخصائي مختبر' : (isSecretary ? 'سكرتير واستقبال طبي' : 'موظف خدمة عملاء');
