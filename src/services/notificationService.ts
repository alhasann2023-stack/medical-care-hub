...
class NotificationService {
  ...
  async sendPaymentSuccessNotification(payment: any): Promise<NotificationDeliveryResult[]> {
    return this.dispatch({
      ...
      message: `تم سداد مبلغ ${payment.amount} ${payment.currency || 'ر.ي'} للخدمة [${payment.serviceName}] عبر ${payment.paymentMethod}. الرقم المرجعي: ${payment.transactionReference}. الموعد مثبت في جدول العيادة.`,
      type: 'PAYMENT',
  ...
  async sendRefundNotification(refund: any): Promise<NotificationDeliveryResult[]> {
    return this.dispatch({
      ...
      message: `تم معالجة استرداد مبلغ ${refund.amount} ${refund.currency || 'ر.ي'} لعملية الدفع ${refund.transactionReference}. سبب الاسترداد: ${refund.reason}.`,
      type: 'REFUND',
