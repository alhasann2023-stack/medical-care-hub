...
export const firebaseDb = {
  ...
  saveUserCredential: async (
    ...
    const id = normalizeId(credential?.userId);
    if (!id || !credential?.password) return false;
  ...
  getUserCredential: async (
    ...
    return await firebaseDb.getDocument<{
      userId: string;
      email?: string;
      phone?: string;
      password?: string;
  ...
  saveAuditLog: async (
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        if (existingIdx >= 0) {
          list[existingIdx] = auditLog;
        } else {
          list.unshift(auditLog);
        }
        localStorage.setItem('audit_logs_cache', JSON.stringify(list.slice(0, 200)));
      }
    } catch {}
  ...
  seedInitialDataIfEmpty:
    async (): Promise<boolean> => {
      ...
      try {
        ...
        const existingAuditLogs =
          await fetchDocsWithFilter<AuditLog>(
            COLLECTIONS.AUDIT_LOGS
          );

        if (
          existingAuditLogs.length === 0
        ) {
          console.log(
            '[Firestore] Seeding initial audit logs trail...'
          );

          for (
            const log of INITIAL_AUDIT_LOGS
          ) {
