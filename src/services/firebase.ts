...
/**
 * Save settings document (e.g. freeConsultationPromo) to Firestore
 */
export async function saveSettingsDoc<T extends Record<string, any>>(settingId: string, data: T): Promise<boolean> {
  if (!settingId || !data) return false;
  try {
    const docRef = doc(db, FIRESTORE_COLLECTIONS.SETTINGS, settingId.trim());
    await setDoc(docRef, {
      ...data,
      updatedAt: new Date().toISOString()
    }, { merge: true });
    ...
    console.warn(`[Firestore] saveSettingsDoc error (${settingId}):`, err);
    return false;
  }
}

/**
 * Fetch settings document (e.g. freeConsultationPromo) from Firestore
 */
export async function getSettingsDoc<T>(settingId: string): Promise<T | null> {
  if (!settingId) return null;
  return await fetchDocById<T>(FIRESTORE_COLLECTIONS.SETTINGS, settingId.trim());
}
