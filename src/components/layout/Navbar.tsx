...
export const Navbar = ({
  ...
  const roleConfigs: Record<UserRole, { label: string; icon: any; color: string; bg: string }> = {
    ...
    SECRETARY: {
      label: 'بوابة السكرتارية والاستقبال',
      icon: Printer,
      color: 'text-teal-700',
      bg: 'bg-teal-50 border-teal-200'
    },
    LAB_TECHNICIAN: {
      label: 'بوابة المختبر والتحاليل الطبية',
      icon: FlaskConical,
      color: 'text-cyan-700',
      bg: 'bg-cyan-50 border-cyan-200'
    },
    RADIOLOGY: {
      label: 'بوابة قسم الأشعة والتصوير الطبي',
      icon: Scan,
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-18">
          ...
          <nav className="hidden lg:flex items-center gap-1 text-sm font-semibold text-slate-600">
            ...
            {role === 'SECRETARY' && (
              <button
                onClick={() => setCurrentView('secretary_dashboard')}
                className="px-3 py-2 rounded-lg text-teal-700 bg-teal-50/80 font-bold"
              >
            ...
            {role === 'LAB_TECHNICIAN' && (
              <button
                onClick={() => setCurrentView('lab_dashboard')}
                className="px-3 py-2 rounded-lg text-cyan-700 bg-cyan-50/80 font-bold"
              >
                بوابة المختبر والتحاليل الطبية
              </button>
            )}
            ...
            {role === 'RADIOLOGY' && (
              <button
                onClick={() => setCurrentView('lab_dashboard')}
                className="px-3 py-2 rounded-lg text-indigo-700 bg-indigo-50/80 font-bold"
              >
                بوابة قسم الأشعة والتصوير الطبي
              </button>
            )}
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-200 bg-white px-4 pt-2 pb-4 space-y-2 shadow-lg">
          ...
          {role === 'SECRETARY' && (
            <button
              onClick={() => { setCurrentView('secretary_dashboard'); setIsMobileMenuOpen(false); }}
              className="w-full text-start px-3 py-2 rounded-md text-sm font-medium text-teal-700 bg-teal-50 font-bold"
            >
              بوابة السكرتارية والاستقبال
            </button>
          )}
          ...
          {role === 'LAB_TECHNICIAN' && (
            <button
              onClick={() => { setCurrentView('lab_dashboard'); setIsMobileMenuOpen(false); }}
              className="w-full text-start px-3 py-2 rounded-md text-sm font-medium text-cyan-700 bg-cyan-50 font-bold"
            >
              بوابة المختبر والتحاليل الطبية
            </button>
          )}
          ...
          {role === 'RADIOLOGY' && (
            <button
              onClick={() => { setCurrentView('lab_dashboard'); setIsMobileMenuOpen(false); }}
              className="w-full text-start px-3 py-2 rounded-md text-sm font-medium text-indigo-700 bg-indigo-50 font-bold"
            >
              بوابة قسم الأشعة والتصوير الطبي
            </button>
          )}
