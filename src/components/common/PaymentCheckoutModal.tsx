  ...
  const handlePaySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedMethod === 'ONE_CASH' || selectedMethod === 'MAHFAZATI' || selectedMethod === 'JEEB' || selectedMethod === 'FLOOSAK' || selectedMethod === 'JAWALI') {
        ...
        const currentWalletName = walletNames[selectedMethod] || 'المحفظة الإلكترونية';
        setProcessingStep(`جارٍ الاتصال بمحفظة ${currentWalletName} والتحقق من حساب (${walletPhone})...`);
        await new Promise(r => setTimeout(r, 600));

        const intentRes = await apiClient.createPaymentIntent({
          patientId,
          patientName: walletCustomerName || patientName,
          patientPhone: walletPhone || patientPhone,
          patientMrn,
          serviceType,
          serviceReferenceId,
          serviceName,
        ...
        notificationService.sendPaymentSuccessNotification(confirmedPayment);
              ...
              <div className="mt-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-400/80 dark:border-slate-700/60 flex items-center justify-between text-[11px] text-slate-600 dark:text-slate-400">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-[10px]">
                    1 $ = {liveRates.USD_TO_YER.toLocaleString()} ر.ي | 1 ر.س = {liveRates.SAR_TO_YER} ر.ي
                  </span>
                </div>
                <button
                  type="button"
              ...
              <>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    ...
                    {selectedCurrency === 'YER' && (
                      <>
                        ...
                        <button
                          ...
                          className={`flex flex-col items-center justify-center p-3 rounded-2xl border text-center transition-all cursor-pointer ${
                            selectedMethod === 'JEEB'
                              ? 'border-sky-500 bg-sky-500/10 dark:bg-sky-950/40 text-sky-900 dark:text-sky-100 ring-2 ring-sky-500/20 shadow-xs'
                              : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:border-slate-300'
                          }`}
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => setSelectedMethod('FLOOSAK')}
                          className={`flex flex-col items-center justify-center p-3 rounded-2xl border text-center transition-all cursor-pointer ${
                            selectedMethod === 'FLOOSAK'
                              ? 'border-indigo-500 bg-indigo-500/10 dark:bg-indigo-950/40 text-indigo-900 dark:text-indigo-100 ring-2 ring-indigo-500/20 shadow-xs'
                              : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:border-slate-300'
                          }`}
                        >
                    ...
                    {/* Apple Pay (available for USD or SAR) */}
                    {selectedCurrency !== 'YER' && (
                      <button
                        type="button"
