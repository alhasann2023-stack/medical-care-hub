...
=======
  ...
  // Keep the form synchronized with the latest patient profile. This is important
  // on Android/WebView where the dashboard can receive the Firestore profile after
  // this component has already been mounted once.
  useEffect(() => {
    if (!patient) return;
    setAllergies(Array.isArray(patient.allergies) ? patient.allergies : []);
    setChronicDiseases(Array.isArray(patient.chronicDiseases) ? patient.chronicDiseases : []);
    setEmergencyName(patient.emergencyContact?.name || '');
    setEmergencyRelation(patient.emergencyContact?.relation || 'قريب');
