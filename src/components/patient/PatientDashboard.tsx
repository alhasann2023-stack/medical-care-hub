...
export const PatientDashboard: React.FC<PatientDashboardProps> = ({
  ...
  const [currentPatient, setCurrentPatient] = useState<Patient | null>(patientProfile || null);
  const [isMedicalProfileModalOpen, setIsMedicalProfileModalOpen] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    const unsubPromo = api.subscribeFreeConsultationPromo((promo) => {
      setFreePromo(promo);
    });
  ...
  const loadDashboardData = async () => {
    setIsLoading(true);
    try {
      const [aptRes, cnsRes, tstRes, repRes, rxRes, docsRes, followUpRes, patRes] = await Promise.all([
      ...
      if (patRes) {
        setCurrentPatient(patRes);
      } else if (patientProfile) {
        setCurrentPatient(patientProfile);
      }

      setAppointments(aptRes);
      setConsultations(cnsRes as any);

      // Strict privacy filter: ensure patient only sees their own tests
      const allowedPatIds = new Set(
        [
          patientProfile?.id,
      ...
      const safePatientTests = (tstRes || []).filter(t => {
        ...
        return (
          (tPatId && allowedPatIds.has(tPatId)) ||
          (tUserId && allowedPatIds.has(tUserId)) ||
          (tMrn && allowedPatIds.has(tMrn))
        );
      });
      setTests(safePatientTests);

      setReports(repRes);
      setPrescriptions(rxRes);
      setDoctorsList(docsRes);
      setFollowUps(followUpRes);
      checkForReminders(aptRes, cnsRes as any, docsRes);
    } catch (err) {
  ...
  const hasUsedFreeConsultation = useMemo(() => {
    const usedInConsultations = consultations.some(c => 
      c.isWaived || 
    ...
    const usedInStorage = typeof window !== 'undefined' && Boolean(
      (patientId && localStorage.getItem(`mch_free_cns_used_${patientId}`) === 'true') ||
      (patientProfile?.phone && localStorage.getItem(`mch_free_cns_used_${patientProfile.phone.replace(/\D/g, '')}`) === 'true') ||
      (user?.phone && localStorage.getItem(`mch_free_cns_used_${user.phone.replace(/\D/g, '')}`) === 'true')
    );
    return usedInConsultations || usedInStorage;
  }, [consultations, patientId, patientProfile?.phone, user?.phone]);

  const isFreePromoActive = Boolean(
    freePromo &&
    freePromo.isActive &&
    (!freePromo.endDate || new Date(freePromo.endDate).getTime() > Date.now()) &&
    !hasUsedFreeConsultation
  );

  return (
    <div className="space-y-6">
      {/* Free Consultations Promotion Banner (Before Usage) */}
      {isFreePromoActive && freePromo && (
        <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 rounded-3xl p-4 sm:p-5 text-white shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4 border border-emerald-400/40">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-black bg-amber-300 text-amber-950">
                  {freePromo.bannerBadgeText || 'مبادرة مجانية 100%'}
                </span>
                <h3 className="font-black text-sm sm:text-base">{freePromo.occasionTitle || 'استشارات طبية مجانية'}</h3>
              </div>
              <p className="text-xs text-emerald-50 mt-1">
                {freePromo.occasionDescription || 'استشارة مجانية واحدة لكل مريض لمرة واحدة، والاستشارات اللاحقة مدعومة.'}
              </p>
            </div>
          </div>
          <button
            onClick={onOpenConsultation}
            className="px-5 py-2.5 rounded-xl bg-white hover:bg-emerald-50 text-emerald-950 font-black text-xs sm:text-sm transition-all shadow-md flex items-center gap-2 cursor-pointer shrink-0"
          >
            <Sparkles className="w-4 h-4 text-emerald-600" />
            <span>طلب استشارة مجانية الآن</span>
          </button>
        </div>
      )}

      {/* Free Promo Consumed Notice Banner (After Usage) */}
      {freePromo && freePromo.isActive && hasUsedFreeConsultation && (
        <div className="bg-slate-900 text-white rounded-3xl p-4 sm:p-5 shadow-md flex flex-col sm:flex-row items-center justify-between gap-4 border border-slate-700">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-slate-800 text-cyan-300 border border-cyan-400/20">
                  استشارة مدعومة
                </span>
                <h3 className="font-bold text-sm sm:text-base">تم الاستفادة من استشارتك المجانية ضمن المبادرة</h3>
              </div>
              <p className="text-xs text-slate-300 mt-1">
                تم استهلاك الاستشارة المجانية المخصصة لحسابك مسبقاً. استشاراتك الحالية متاحة بالرسوم المدعومة الرسمية.
              </p>
            </div>
          </div>
          <button
            onClick={onOpenConsultation}
            className="px-4 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-xs sm:text-sm transition-all shadow-md flex items-center gap-2 cursor-pointer shrink-0"
          >
            <Sparkles className="w-4 h-4" />
            <span>طلب استشارة مدعومة</span>
          </button>
        </div>
      )}
      ...
      <div className="bg-gradient-to-r from-blue-700 via-blue-800 to-indigo-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              onClick={onOpenConsultation}
              className={`px-4 py-2.5 rounded-xl font-extrabold text-xs sm:text-sm transition-all shadow-md flex items-center gap-2 cursor-pointer ${
                isFreePromoActive
                  ? 'bg-amber-400 hover:bg-amber-300 text-slate-950 shadow-amber-500/20 animate-pulse'
            ...
            >
              {isFreePromoActive ? (
                <>
                  ...
                  <span>استشر طبيباً (مجاناً 100%)</span>
                </>
              ) : (
                <>
                  <Sparkles className="w-4 h-4 text-slate-950" />
                  <span>استشر طبيباً {hasUsedFreeConsultation ? '(مدعومة)' : ''}</span>
                </>
              )}
            </button>
            <button
      ...
      {/* 6 Feature Shortcut Cards (حجز موعد، استشارة، السجل الزمني، فحوصاتي، تقاريري، الفواتير) */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
        ...
        <button
          onClick={onOpenConsultation}
          className={`p-4 rounded-2xl bg-white border shadow-xs hover:shadow-md transition-all text-start group cursor-pointer relative ${
            isFreePromoActive ? 'border-amber-400 ring-1 ring-amber-400/50 bg-amber-50/20' : 'border-slate-200 hover:border-cyan-300'
          }`}
        >
          {isFreePromoActive && (
            <span className="absolute top-3 left-3 px-2 py-0.5 rounded-full text-[10px] font-black bg-amber-400 text-slate-950 shadow-xs">
              مجاناً
            </span>
          )}
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {isFreePromoActive ? 'متاحة مجاناً الآن' : `${consultations.length} استشارات مسجلة`}
          </p>
        </button>
        ...
        >
          ...
          <p className="text-xs text-slate-500 mt-0.5">خط زمني طبي متكامل</p>
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-amber-300 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              ...
              {upcomingAppointment && (
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                  upcomingAppointment.isDoctorAbsent
            ...
            {upcomingAppointment ? (
              <div className="p-4 rounded-2xl bg-gradient-to-br from-slate-50 to-blue-50/40 border border-slate-400">
                {upcomingAppointment.isDoctorAbsent && (
                  <div className="mb-3.5 p-3.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-900 text-xs flex items-start gap-2.5 shadow-2xs">
                    <AlertTriangle className="w-5 h-5 text-rose-600 shrink-0 mt-0.5" />
                    <div>
                      <strong className="block text-xs font-black text-rose-950 mb-0.5">
                        إشعار من خدمة العملاء: الطبيب غير مداوم في العيادة
                      </strong>
        ...
        {/* Right 1 Col: Emergency Medical Data, Recent Consultation & Fast Navigation */}
        <div className="space-y-6">
          {/* Medical & Vital Emergency Data Card (Added by Patient) */}
          <div className="bg-white rounded-2xl p-6 border border-blue-500 shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-slate-00 pb-3">
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-xl bg-rose-50 text-rose-600">
                  <Heart className="w-5 h-5" />
                </div>
                <div>
                  <h2 className="font-extrabold text-base text-slate-900">البيانات الطبية والحيوية</h2>
                  <span className="text-[11px] text-blue-600 font-medium">بيانات الحساسية والأمراض المزمنة للطوارئ</span>
                </div>
              </div>
              <button
            ...
            {/* 1. Drug & Food Allergies */}
            <div className="space-y-1.5 text-xs">
              <span className="font-extrabold text-slate-900 flex items-center gap-1.5 text-rose-700">
              ...
              {currentPatient?.allergies && currentPatient.allergies.length > 0 ? (
                <div className="flex flex-wrap gap-1 pt-1">
                  {currentPatient.allergies.map((alg, i) => (
                    <span
                      key={i}
                      className="px-2.5 py-0.5 rounded-md bg-rose-50 text-rose-800 border border-rose-200 text-[11px] font-bold"
                    >
                      {alg}
                    </span>
                  ))}
                ...
                <p className="text-slate-500 text-[11px] pr-5 font-medium">
            ...
            <div className="space-y-1.5 text-xs pt-1 border-t border-slate-100">
              <span className="font-extrabold text-slate-900 flex items-center gap-1.5 text-amber-700">
                <Heart className="w-3.5 h-3.5" />
                <span>الأمراض والحالات المزمنة:</span>
              </span>
              {currentPatient?.chronicDiseases && currentPatient.chronicDiseases.length > 0 ? (
                <div className="flex flex-wrap gap-1 pt-1">
                  {currentPatient.chronicDiseases.map((dis, i) => (
                    <span
                      key={i}
                      className="px-2.5 py-0.5 rounded-md bg-amber-50 text-amber-800 border border-amber-200 text-[11px] font-bold"
                    >
                      {dis}
                    </span>
                  ))}
            ...
            {/* 3. Emergency Contact */}
            <div className="space-y-1.5 text-xs pt-1 border-t border-slate-100 bg-slate-50/70 p-3 rounded-xl border border-slate-100">
              <span className="font-extrabold text-slate-900 flex items-center gap-1.5 text-blue-800">
                <Phone className="w-3.5 h-3.5" />
                <span>جهة الاتصال في حالات الطوارئ:</span>
              </span>
              <div className="text-[11px] text-slate-600 space-y-1 pr-5">
                <div>
                  <span className="text-slate-400">الاسم: </span>
                  <strong className="text-slate-900">{currentPatient?.emergencyContact?.name || 'جهة اتصال الطوارئ'}</strong>
                </div>
                <div>
                  <span className="text-slate-400">الصلة: </span>
                  <strong className="text-slate-900">{currentPatient?.emergencyContact?.relation || 'قريب'}</strong>
                </div>
                <div>
                  <span className="text-slate-400">الهاتف: </span>
                  <strong className="text-blue-700 font-mono" dir="ltr">{currentPatient?.emergencyContact?.phone || '+966509998877'}</strong>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 border border-green-300 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2.5">
              ...
              <div className="space-y-3">
                {sortedConsultations.slice(0, 3).map((c) => (
                  <div
                    ...
                    className="p-4 rounded-xl bg-slate-50 hover:bg-slate-100/80 border border-blue-300 text-xs space-y-2.5 transition-all cursor-pointer group"
                  >
                    <div className="flex items-center justify-between">
          ...
          <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200 text-start">
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto text-xs sm:text-sm">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
                ...
                {selectedConsultation.attachments && Array.isArray(selectedConsultation.attachments) && selectedConsultation.attachments.length > 0 && (
                  <div className="pt-2 border-t border-slate-200/80 space-y-1.5">
                    <strong className="block text-slate-800 font-bold text-[11px] flex items-center gap-1">
                      <Paperclip className="w-3.5 h-3.5 text-cyan-600" />
                      المرفقات والملفات المرفوعة ({selectedConsultation.attachments.length}):
                    </strong>
                      ...
                      {selectedConsultation.attachments.map((att: any, idx: number) => {
                        ...
                        return (
                          <a
                            ...
                            download={att.name || `file-${idx + 1}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            onClick={(e) => {
                              if (!hasValidUrl) e.preventDefault();
                            }}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white hover:bg-cyan-50 border border-cyan-200 text-cyan-900 text-xs font-bold transition-colors cursor-pointer shadow-2xs"
                            title={hasValidUrl ? 'انقر لتنزيل أو فتح الملف' : 'ملف مرفق'}
                          >
                            <FileText className="w-3.5 h-3.5 text-cyan-700 shrink-0" />
                            <span className="truncate max-w-[170px]">{att.name || 'ملف مرفق'}</span>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
                        c.status === 'ANSWERED' || Boolean(c.doctorAdvice)
                          ? 'bg-emerald-100 text-emerald-800 border border-emerald-200'
                          : 'bg-amber-100 text-amber-800 border border-amber-200'
                      }`}>
                        {c.status === 'ANSWERED' || Boolean(c.doctorAdvice) ? 'تم الرد من الطبيب ✓' : 'قيد المراجعة'}
                      </span>
      ...
      {/* Patient Medical & Vital Profile Modal */}
      {isMedicalProfileModalOpen && (
        <PatientMedicalProfileModal
          ...
          patient={currentPatient || patientProfile}
          onSuccess={(updated) => {
