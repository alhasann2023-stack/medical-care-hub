...
export const MedicalTimeline: React.FC<MedicalTimelineProps> = ({
  ...
  const effectivePatientId = patientId || patientProfile?.id || user?.id || 'pat-1';

  const loadTimeline = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await api.getPatientTimeline(effectivePatientId);
      setPatient(res.patient || patientProfile || null);
  ...
  }, [effectivePatientId, patientProfile]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative border-r-2 border-slate-200 mr-5 sm:mr-8 pr-5 sm:pr-8 space-y-6">
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-white rounded-2xl border border-slate-200 text-slate-400 p-8">
            ...
            <p className="text-sm font-semibold mb-2">لا توجد سجلات تطابق الفلتر المحدد.</p>
            <button
              onClick={() => { setSelectedFilter('ALL'); setSearchQuery(''); loadTimeline(); }}
          ...
          filteredTimeline.map((item: TimelineItem) => {
            ...
            return (
              <div key={item.id} className="relative group">
                {/* Node Bullet Icon - perfectly centered on right border */}
                <div
                  className={`absolute -right-[27px] sm:-right-[29px] top-4 w-7 h-7 sm:w-8 sm:h-8 rounded-full ${config.bg} ${config.color} border-2 border-white shadow-sm flex items-center justify-center transition-transform group-hover:scale-110 z-10`}
                >
