...
export const PatientFileModal: React.FC<PatientFileModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-5">
            ...
            <>
              ...
              {activeTab === 'OVERVIEW' && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="p-4 rounded-2xl bg-blue-50 border border-blue-200 space-y-1.5">
                      ...
                      <div className="text-xs text-blue-950 space-y-0.5">
                        <p><strong>الاسم:</strong> {patient?.emergencyContact?.name || 'جهة اتصال الطوارئ'}</p>
                  ...
                  {(() => {
                    const latestExamWithVitals = examinations.find(e => e.vitalSigns) || examinations[0];
                    const currentVitals = latestExamWithVitals?.vitalSigns;
                    const bpVal = currentVitals?.bloodPressure || '120/80';
                    const hrVal = currentVitals?.heartRate ? String(currentVitals.heartRate) : '72';
                    const tempVal = currentVitals?.temperature ? `${currentVitals.temperature}°C` : '37.0°C';
                    const o2Val = currentVitals?.oxygenSaturation ? `${currentVitals.oxygenSaturation}%` : '99%';
                    ...
                    return (
                      <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200 space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          {latestExamWithVitals && (
                            <span className="text-[11px] text-slate-500 font-medium">
                              (المسجلة في المعاينة السريرية بتاريخ: <span className="font-mono text-slate-700 font-bold">{latestExamWithVitals.examinationDate}</span>)
                            </span>
                          )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          <div className="bg-white p-3 rounded-xl border border-slate-200 text-center shadow-2xs">
                            ...
                            <strong className="text-base text-slate-900 font-mono">{bpVal}</strong>
                          ...
                          <div className="bg-white p-3 rounded-xl border border-slate-200 text-center shadow-2xs">
                            ...
                            <strong className="text-base text-slate-900 font-mono">{hrVal}</strong>
                          ...
                          <div className="bg-white p-3 rounded-xl border border-slate-200 text-center shadow-2xs">
                            ...
                            <strong className="text-base text-slate-900 font-mono">{tempVal}</strong>
                          ...
                          <div className="bg-white p-3 rounded-xl border border-slate-200 text-center shadow-2xs">
                            ...
                            <strong className="text-base text-slate-900 font-mono">{o2Val}</strong>
              ...
              {activeTab === 'TESTS' && (
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200">
                    <div>
                      <h3 className="font-extrabold text-sm text-slate-900">
                        سجل نتائج الفحوصات والتحاليل المخبرية والأشعة ({tests.length})
                      </h3>
                      <p className="text-[11px] text-slate-500 mt-0.5">
                        استعراض نتائج التحاليل والفحوصات المعتمدة والمرسلة من قِبل فني وأخصائي المختبر
                      </p>
                    </div>
                  </div>
                  ...
                  {tests.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {tests.map((t) => {
                        const isImage = t.attachmentUrl && (
                          t.attachmentUrl.startsWith('data:image/') || 
                          t.attachmentUrl.startsWith('http') && /\.(jpg|jpeg|png|webp)/i.test(t.attachmentUrl) ||
                          /\.(jpg|jpeg|png|webp)/i.test(t.attachmentName || '')
                        );
                        ...
                        return (
                          <div key={t.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
                            ...
                            {/* Attached File Preview or Download */}
                            {t.attachmentUrl && t.attachmentUrl !== '#' && (
                              <div className="p-3.5 rounded-xl bg-emerald-50/60 border border-emerald-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div className="flex items-center gap-3">
                                  {isImage ? (
                                    <div 
                                    ...
                                    >
                                      <img
                                        src={t.attachmentUrl}
                                        alt={t.attachmentName || t.testName}
                                  ...
                                  <div className="text-xs">
                                    <span className="font-bold text-emerald-950 block">
                                      {t.attachmentName || 'ملف الأشعة / الفحص المرفق'}
                                    </span>
                                    <span className="text-[10px] text-emerald-700">
                                      {isImage ? 'صورة أشعة تشخيصية (اضغط للتكبير)' : 'مستند فحص طبي'}
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                  {isImage && (
                                    <button
                                      type="button"
                                      onClick={() => setPreviewImage(t.attachmentUrl!)}
                                      className="px-3 py-1.5 rounded-lg bg-white hover:bg-emerald-100 text-emerald-900 text-xs font-bold border border-emerald-300 flex items-center gap-1.5 transition-colors cursor-pointer"
                                    >
                                      <Eye className="w-3.5 h-3.5" />
                                      <span>معاينة الأشعة</span>
                                    </button>
                                  )}
                                  <a
                                    href={t.attachmentUrl}
                                    download={t.attachmentName || `${t.testName}.pdf`}
                                    target="_blank"
                                    rel="noreferrer"
                                    className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shadow-2xs"
                                  >
                            ...
                            <div className="flex items-center justify-between text-xs text-slate-500 pt-2">
                              <span>الفني المسؤول: {t.labTechnician || 'قسم المختبر والأشعة'}</span>
      ...
      {/* Full Size Image / X-Ray Lightbox */}
      {previewImage && (
        <div 
          ...
          >
            <div className="p-4 bg-slate-950/80 text-white flex items-center justify-between border-b border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <span className="font-extrabold text-sm">معاينة صورة الأشعة / الفحص الطبي التشخيصي</span>
              </div>
              <div className="flex items-center gap-2">
                <a
                  href={previewImage}
                  download="xray_scan_image.png"
            ...
            <div className="flex-1 overflow-auto p-4 flex items-center justify-center bg-black/40">
              <img
                src={previewImage}
                alt="الأشعة السينية / الفحص الطبي"
