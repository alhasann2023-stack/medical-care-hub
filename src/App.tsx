...
const MainAppContent: React.FC = () => {
  ...
  const handleRoleSelectFromHero = async (selectedRole: UserRole) => {
    ...
    else if (selectedRole === 'SECRETARY') setCurrentView('secretary_dashboard');
    else if (selectedRole === 'LAB_TECHNICIAN' || selectedRole === 'RADIOLOGY') setCurrentView('lab_dashboard');
    else if (selectedRole === 'HOSPITAL_ADMIN') setCurrentView('admin_dashboard');
  };

  useEffect(() => {
    if (role === 'SECRETARY' && (currentView === 'landing' || currentView === 'patient_dashboard')) {
      setCurrentView('secretary_dashboard');
    } else if ((role === 'LAB_TECHNICIAN' || role === 'RADIOLOGY') && (currentView === 'landing' || currentView === 'patient_dashboard')) {
      setCurrentView('lab_dashboard');
    }
  }, [role]);
  ...
  const handleNavigateFromNotification = (type: string) => {
    ...
    else if (role === 'SECRETARY') setCurrentView('secretary_dashboard');
    else if (role === 'LAB_TECHNICIAN' || role === 'RADIOLOGY') setCurrentView('lab_dashboard');
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* Secretary View */}
        {currentView === 'secretary_dashboard' && (
          <SecretaryDashboard />
        )}

        {/* Lab Technician View */}
        {currentView === 'lab_dashboard' && (
          <LabDashboard />
        )}
