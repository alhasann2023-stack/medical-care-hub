export type UserRole = 'PATIENT' | 'DOCTOR' | 'CUSTOMER_SERVICE' | 'HOSPITAL_ADMIN' | 'SECRETARY' | 'LAB_TECHNICIAN' | 'RADIOLOGY';
...
export interface WhitelistedFreePatient {
  id: string;
  patientId?: string;
  patientName: string;
  patientPhone?: string;
...
export interface FreeConsultationPromo {
  id: string;
  isActive: boolean;
  ...
  autoExpire: boolean;
  applicableDoctors: 'ALL' | string[];
  totalFreeConsultationsCount?: number;
  oneFreePerPatient?: boolean;
  whitelistedPatients?: WhitelistedFreePatient[];
  updatedAt: string;
  updatedBy?: string;
}
